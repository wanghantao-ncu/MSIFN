...
class BaselineTrain(nn.Module):
    ...
    def train_loop(self, epoch, train_loader, optimizer, count):
        print_freq = 200
        avg_loss = 0
        total_correct = 0
        ...
        for i, (x, y) in enumerate(train_loader):
            y = Variable(y.cuda())
            optimizer.zero_grad()
            #forward_loss中只有celoss，需要加上gaussianloss
            ...
            if i % print_freq == 0:
                print('Epoch {:d} | Batch {:d}/{:d} | Loss {:f}'.format(epoch, i, len(train_loader), train_loss / float(i + 1)))
